...
function initAdminWebSocket() {
  try {
    ...
    adminWs = new WebSocket(window.WS_URL || 'ws://localhost:4000/ws/connect');
...
async function fetchAllUserData() {
  try {
    ...
    const registeredResponse = await fetch(getApiUrl("/admin/users"), {
      method: "GET",
      headers: getAuthHeader(),
      credentials: "include"
    });
    ...
    const connectedResponse = await fetch(getApiUrl("/admin/connected-users"), {
      method: "GET",
      headers: getAuthHeader(),
      credentials: "include"
    });
