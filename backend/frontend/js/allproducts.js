async function fetchAndPopulateCategories() {
    try {
        const genderResponse = await fetch(getApiUrl("/category?is_gender=true"));
        const typeResponse = await fetch(getApiUrl("/category?is_gender=false"));
...
function applyFilters() {
    ...
    // Make request
    fetch(getApiUrl(`/products/filter?${params.toString()}`))
      .then(res => res.json())
      .then(data => {
...
async function loadAllProducts() {
    try {
        ...
        let endpoint;
        if (categoryId) {
            endpoint = getApiUrl(`/products/gender?CategoryId=${categoryId}`); // Fetch products by gender
        } else if (typeId) {
            endpoint = getApiUrl(`/products/type?TypeId=${typeId}`); // Fetch products by type
        } else {
            endpoint = getApiUrl('/allproducts'); // Fetch all products
        }
